...
func (s *Scanner) scanVarEnds(left, right rune) {
	wait := 1
	for {
		switch s.ch {
		case -1:
			s.errorf(s.offset, "expect %c", right)
			return
		case right:
			s.next()
			wait--
...
func (s *Scanner) scanVars() (tok Token, lit string) {
	offs := s.offset
	switch s.ch {
	case '$':
		s.next()
...
func (s *Scanner) Scan() (pos int, tok Token, lit string) {
	...
	default:
		...
		case '&':
			if s.ch == '&' {
				...
				tok = ILLEGAL
				s.errorf(s.rdOffset, "invalid operator &%c", s.ch)
			}
		case '|':
			if s.ch == '|' {
				s.next()
		...
		default:
			...
			if ch != bom {
				s.errorf(s.rdOffset, "illegal character %#U", ch)
			}
			tok = ILLEGAL
			lit = string(ch)
		}
	...
	//fmt.Printf("scan out, token: %d lit %s\n", tok, lit)
	return
}
